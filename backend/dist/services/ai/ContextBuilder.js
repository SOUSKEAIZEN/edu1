...
class ContextBuilder {
    static detectIntent(query) {
        ...
        if (q.includes('mark') || q.includes('grade') || q.includes('gpa') || q.includes('failing'))
            return 'ACADEMIC_QUERY';
        if (q.includes('attendance') || q.includes('absent') || q.includes('missed class'))
            return 'ATTENDANCE_QUERY';
        if (q.includes('task') || q.includes('deadline') || q.includes('due') || q.includes('homework'))
            return 'TASK_QUERY';
        return 'GENERAL_MENTORING';
    }
    static async buildContext(studentUserId, query) {
        ...
        const analytics = await analytics_service_1.AnalyticsService.getStudentAnalytics(studentUserId);
        const createMetadata = (source) => ({
            source,
            timestamp: analytics.calculated_at,
            freshnessMinutes: Math.floor((now.getTime() - new Date(analytics.calculated_at).getTime()) / 60000),
            permissionScope: 'STUDENT_ONLY'
        });
