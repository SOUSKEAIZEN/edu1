...
function calculateTrend(assessments: { marks_obtained: number, max_marks: number, weight: number }[]) {
...
test('Analytics Engine Math - Academic Trend', (t) => {
  const syntheticData = [
  ...
  assert.strictEqual(result.recentAvg, 85);
  assert.strictEqual(result.histAvg, 55);
  assert.strictEqual(result.improvement, 30);
  assert.strictEqual(result.overallAverage, 65);
});
