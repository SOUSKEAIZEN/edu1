export type MentorMode = 'LEARN' | 'ANALYZE' | 'PLAN' | 'PRACTICE' | 'REVIEW' | 'MENTOR' | 'REFLECT';

export class PromptEngine {
  
  static getSystemPrompt(mode: MentorMode = 'LEARN', version: string = 'v1'): string {
...
`;
  ...
  static constructPrompt(intent: string, contextString: string, userQuery: string, history: any[] = []): string {
    let historyString = '';
    if (history.length > 0) {
