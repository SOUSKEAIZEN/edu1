...
export class MentorDashboardService {
  ...
  static async getDashboardMetrics(mentorUserId: string) {
    const profileRes = await query('SELECT id FROM mentor_profiles WHERE user_id = $1', [mentorUserId]);
    if (profileRes.rows.length === 0) throw new Error('Mentor profile not found');
  ...
  static async getStudentTable(mentorUserId: string) {
    const profileRes = await query('SELECT id FROM mentor_profiles WHERE user_id = $1', [mentorUserId]);
    const mentorId = profileRes.rows[0].id;
